	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseNormalClosure, websocket.CloseGoingAway, websocket.CloseAbnormalClosure, websocket.CloseNoStatusReceived) {
				log.Printf("CloseError: %v", err)
			}else{
				// log.Printf("uid: %v has logout", c.uid)
			}
			break
		}
		// log.Printf("n:%v,c:%v,msg:%v",n,c.uid,string(message))
...
func ServeWs(room *Room, w http.ResponseWriter, r *http.Request, uid string) {
	conn, err := upgrader.Upgrade(w, r, nil)
	if err != nil {
		log.Println(err)
		return
	}
	client := &Client{room: room,uid: uid, conn: conn, send: make(chan []byte, 256)}
